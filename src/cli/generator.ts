...
import {
    ...
    DrumNote,
    isDrumNote,
    isPatternReference, isTimeSignatureChange, TimeSignatureChange,
...
function fillStacks(notes: (ClassicNote|DrumNote|TimeSignatureChange)[], tickCount: number) {
    ...
    notes.forEach(note => {
        ...
        if (isClassicNote(note) || isDrumNote(note)) {
            ...
            console.log(`ADD time signature change: ${note.numerator}/${note.denominator}`)
            MIDI_ON_Stack.push({
    ...
    while (MIDI_ON_Stack.length > 0 || MIDI_OFF_Stack.length > 0) {
        ...
        if (on?.tickMark < off?.tickMark) {
            ...
            if (isTimeSignatureChange(note?.note)) {
                console.log(`time signature change: ${note?.note.numerator}/${note?.note.denominator}`)
                trackMidi.addEvent(new MidiWriter.TimeSignatureEvent(note?.note.numerator!, note?.note.denominator!, 24, 8));
            } else {
...
function generateDrumsEvents(trackMidi: MidiWriter.Track, drumNotes: (DrumNote[]|TimeSignatureChange)[], ticks: number) {
    drumNotes.forEach(note => {
        if (isTimeSignatureChange(note)) {
            console.log(`time signature change: ${note.numerator}/${note.denominator}`)
            trackMidi.addEvent(new MidiWriter.TimeSignatureEvent(note.numerator, note.denominator, 24, 8));
            return;
        }
...
export function generateJavaScript(
    ...
    model.music.tracks.forEach((track: Track) => {
        ...
        if (track.instrument.instrument === 'Drums') {
            // Drums
            let drumNotes = track.notes.filter(note => isDrumNote(note) || isTimeSignatureChange(note)) as (DrumNote|TimeSignatureChange)[];
